...
%
%  Export trial-wise SPN and CNV values from NON-INTERPOLATED datasets.
%
%  Condition labels are parsed from runID:
%    sub01s1 -> in silence
%    sub01n1 -> in noise
%
%  Output:
...
for subNum = subList
    ...
    %% SPN files
    spnFiles = dir(fullfile(outDir, '*_spn_ep_noninterp_base.set'));
    ...
    for i = 1:numel(spnFiles)
        ...
        runID = erase(spnFiles(i).name, '_spn_ep_noninterp_base.set');
    ...
    %% CNV files
    cnvFiles = dir(fullfile(outDir, '*_cnv_ep_noninterp_base.set'));
    ...
    for i = 1:numel(cnvFiles)
        ...
        runID = erase(cnvFiles(i).name, '_cnv_ep_noninterp_base.set');
...
% ------------------------------
if isempty(allRows)
    error('No rows were exported. Check *_spn_ep_noninterp_base.set and *_cnv_ep_noninterp_base.set files.');
end

outFile = fullfile(rootDir, 'anticipatory_electrode_long_noninterp.csv');
